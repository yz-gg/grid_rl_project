...
from trainers.train_sac_dummy import build_agent, build_env
from utils import ensure_dir, load_config, plot_evaluation, set_seed, get_device
...
def evaluate(config_path, model_path=None) -> None:
    ...
    if model_path is None:
        model_path = eval_config.get("model_path", "checkpoints/sac_dummy_final.pt")
    ...
    plot_path = result_dir / eval_config.get("plot_file", "sac_dummy_evaluation.png")
    csv_path = result_dir / eval_config.get("csv_file", "sac_dummy_evaluation.csv")

    pd.DataFrame(history).to_csv(csv_path, index=False)
    plot_evaluation(history, plot_path)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate SAC on DummyGridEnv.")
